...
public class SimpleAnomalyDetector implements TimeSeriesAnomalyDetector {
	...
	private static float corOffset=0.9f;
	private static float threshOffsetMul=1f;
	
	@Override
	public void learnNormal(TimeSeries ts) { 
		float[] maxArr = new float[ts.getNumOfFeatures()];
		int maxIndex=0;
		for (int i=0; i<maxArr.length;i++) // initializing maxArr
			maxArr[i]=0;
	...
	public float findMax(float[] arr)
	{
		...
		Arrays.sort(temp);
		return temp[arr.length*95/100];
	}
	
	
	//Returns an array of dev valueã of each point to a line.
